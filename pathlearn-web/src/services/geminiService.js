...
function normalizeQuiz(quiz) {
  ...
  const rawContext = quiz.context ?? quiz.contexto
...
function matchQuizzesToPOIs(quizzes, pois) {
  return quizzes.map(quiz => {
    ...
    if (pois.find(p => p.id === normalized.poi_id)) return normalized
    const nameMatch = pois.find(p =>
      p.name.toLowerCase().includes(normalized.poi_name?.toLowerCase()) ||
      normalized.poi_name?.toLowerCase().includes(p.name.toLowerCase())
    )
    return nameMatch ? { ...normalized, poi_id: nameMatch.id } : normalized
  })
}
...
export async function generateQuizzes(pois, location, locationName, mode = 'explorer', apiKey) {
  ...
  const exampleContext = mode === 'historian'
    ? 'Frase de contexto histórico que sitúa al estudiante.'
    : ''
...
JSON EXACTO (sin texto fuera del JSON):
{
  ...
  const quizzesMap = {}
  entries.forEach(e => {
    if (e.quiz?.poi_id) quizzesMap[e.quiz.poi_id] = normalizeQuiz(e.quiz)
  })
...
JSON EXACTO (sin texto fuera del JSON):
{
  ...
  const quizzesMap = {}
  results.forEach(r => {
    if (r.quiz?.poi_id) quizzesMap[r.quiz.poi_id] = normalizeQuiz(r.quiz)
  })
